...
ggplot() +
  geom_spatraster(data = doy_trend_200) +
...
years <- as.numeric(names(gdd_doy_stack_200))
getTrend <- function(x) {
  if (any(is.na(x))) {
    c(slope = NA, p.val = NA)
  } else {
...
slope_rast <- app(gdd_doy_stack_200, getTrend)
...
plot(p_adj <= 0.5)
non_sig <- as.polygons(p_adj <= 0.5) |> filter(lyr.1 == 0)  |> mutate(lyr.1 = as.factor(lyr.1))
...
ggplot() +
  ...
  scale_pattern_fill_manual("p > 0.5", values = c("grey30")) +
