...
# showtext::showtext.auto() #makes fonts work consistently
...
gdd_rast <- 
  project(gdd_doy_stack_50, crs("ESRI:102010"), res = 25000)
...
summary(m_reml) #change back to flextable if possible
```
...
If we want to extract information from this model to create a map similar to @fig-trend-map-hatched, we can calculate the *average* slopes and associated p-values testing if those average slopes are different from zero.
