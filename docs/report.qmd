...
tar_load(c(
  ...
  sd_plot_50, sd_plot_650, sd_plot_2500,
...
summary_plots_orig <- c(summary_plot_50, summary_plot_650, summary_plot_2500)
summary_plots <- 
    file_copy(
        path = here(summary_plots_orig), new_path = path(here("docs", "img"), path_file(summary_plots_orig)),
        overwrite = TRUE
    )

sd_plots_orig <- c(sd_plot_50, sd_plot_650, sd_plot_2500)
sd_plots <- 
    file_copy(
        path = here(sd_plots_orig), new_path = path(here("docs", "img"), path_file(sd_plots_orig)),
        overwrite = TRUE
    )
...
::: callout-important
The color scale for each threshold has a different range.
:::
...
```{r}
knitr::include_graphics(summary_plots)
```

### Standard Deviation

```{r}
knitr::include_graphics(sd_plots)
```
